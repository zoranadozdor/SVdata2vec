...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_stats = evaluate(data_loader_val, model, device, args)
        if(args.train_feeder_args['dataset']=="smarthome"):
            print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats:.1f}%")
            max_accuracy = max(max_accuracy, test_stats)
            if log_writer is not None:
                log_writer.add_scalar('perf/test_acc1', test_stats, epoch)
