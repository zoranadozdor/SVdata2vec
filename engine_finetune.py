...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples,  kps, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
...
def evaluate(data_loader, model, device, args):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.no_grad():
            if(args.train_feeder_args['multimodal']):
                output = model(images,kps)
            else:
                output = model(images)

            loss = criterion(output, target.squeeze())

        acc1, acc5 = accuracy(output, target.squeeze(), topk=(1, 5))
