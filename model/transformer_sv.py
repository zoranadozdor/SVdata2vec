...
class ActionHeadFinetune(nn.Module):
    def __init__(self, dropout_ratio=0., dim_feat=512, num_classes=60, num_joints=25, hidden_dim=2048):
        ...
        self.fc1 = nn.Linear(dim_feat, hidden_dim)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., mlp_out_ratio=1.,
                 ...
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, block_norm_first=True):
        super().__init__()
...
class Transformer(nn.Module):
    def __init__(self, dim_in=3, num_classes=3, dim_feat=256,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, protocol='linprobe', block_norm_first=True):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, block_norm_first=block_norm_first)
    ...
    def forward(self, x, x_skele):
        N, I, C, T, H, W = x.shape

        x = x.view(N, C, T, H, W)
        x=self.feature_extractor_video(x)
